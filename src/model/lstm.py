...
RESULT_DIR = Path(get_relative_path(__file__, f"../../_data/results/"))
...
def normalize_dataset(training_data, test_data, order=2):
    data = np.concatenate((training_data, test_data), axis=0)

    scaled_data = tf.keras.utils.normalize(data[:, 0], axis=-1, order=order)
